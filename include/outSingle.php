...
$perfectmoney = $mysqli_local->query("SELECT in_acc,out_id,out_pass FROM hyip_perfectmoney WHERE active=1")->fetchSingleRow();
...
where acc.account is not null and DATE(hc.created) != CURDATE() and hu.banned=0 AND hc.id=$id LIMIT 1")->fetchAll();
